...
GO
ALTER TABLE [dbo].[PJ_TB_PAYMENTS]  WITH CHECK ADD  CONSTRAINT [CK_PJ_TB_PAYMENTS_PROVIDER] CHECK  (([provider]='PAYPAL_CARD' OR [provider]='PAYPAL' OR [provider]='NAVERPAY' OR [provider]='KAKAOPAY'))
GO
...
IF EXISTS (
	...
	WHERE name = 'CK_PJ_TB_PAYMENTS_PROVIDER'
	  AND parent_object_id = OBJECT_ID('dbo.PJ_TB_PAYMENTS')
)
BEGIN
	ALTER TABLE dbo.PJ_TB_PAYMENTS DROP CONSTRAINT [CK_PJ_TB_PAYMENTS_PROVIDER];
END
GO

ALTER TABLE dbo.PJ_TB_PAYMENTS WITH CHECK
ADD CONSTRAINT [CK_PJ_TB_PAYMENTS_PROVIDER]
CHECK ([provider] IN ('KAKAOPAY', 'NAVERPAY', 'PAYPAL', 'PAYPAL_CARD'));
GO
...
IF COL_LENGTH('dbo.PJ_TB_USERS', 'status') IS NULL
BEGIN
	ALTER TABLE dbo.PJ_TB_USERS
	ADD [status] VARCHAR(20) NOT NULL CONSTRAINT [DF_PJ_TB_USERS_STATUS] DEFAULT ('ACTIVE');
END
GO

IF COL_LENGTH('dbo.PJ_TB_USERS', 'withdrawn_at') IS NULL
BEGIN
	ALTER TABLE dbo.PJ_TB_USERS
	ADD [withdrawn_at] DATETIME2(0) NULL;
END
GO

IF COL_LENGTH('dbo.PJ_TB_USERS', 'withdraw_reason') IS NULL
BEGIN
	ALTER TABLE dbo.PJ_TB_USERS
	ADD [withdraw_reason] NVARCHAR(500) NULL;
END
GO
...
IF NOT EXISTS (
	SELECT 1
	FROM sys.check_constraints
	WHERE name = 'CK_PJ_TB_USERS_STATUS'
	  AND parent_object_id = OBJECT_ID('dbo.PJ_TB_USERS')
)
BEGIN
	ALTER TABLE dbo.PJ_TB_USERS WITH CHECK
	ADD CONSTRAINT [CK_PJ_TB_USERS_STATUS]
	CHECK ([status] IN ('ACTIVE', 'WITHDRAWN', 'BLOCKED'));
END
GO

IF OBJECT_ID('dbo.PJ_TB_SIGNUP_BONUS_HISTORY', 'U') IS NULL
BEGIN
	CREATE TABLE [dbo].[PJ_TB_SIGNUP_BONUS_HISTORY](
		[bonus_id] [bigint] IDENTITY(1,1) NOT NULL,
		[provider] [varchar](20) NOT NULL,
		[login_id] [varchar](200) NOT NULL,
		[user_id] [bigint] NULL,
		[granted_tokens] [int] NOT NULL CONSTRAINT [DF_PJ_TB_SIGNUP_BONUS_HISTORY_TOKENS] DEFAULT ((0)),
		[created_at] [datetime2](0) NOT NULL CONSTRAINT [DF_PJ_TB_SIGNUP_BONUS_HISTORY_CREATED] DEFAULT (sysdatetime()),
	 CONSTRAINT [PK_PJ_TB_SIGNUP_BONUS_HISTORY] PRIMARY KEY CLUSTERED ([bonus_id] ASC)
	);
END
GO
...
IF NOT EXISTS (
	SELECT 1
	FROM sys.indexes
	WHERE object_id = OBJECT_ID('dbo.PJ_TB_SIGNUP_BONUS_HISTORY')
	  AND name = 'UX_PJ_TB_SIGNUP_BONUS_HISTORY_PROVIDER_LOGIN_ID'
)
BEGIN
	CREATE UNIQUE NONCLUSTERED INDEX [UX_PJ_TB_SIGNUP_BONUS_HISTORY_PROVIDER_LOGIN_ID]
	ON [dbo].[PJ_TB_SIGNUP_BONUS_HISTORY] ([provider] ASC, [login_id] ASC);
END
GO
